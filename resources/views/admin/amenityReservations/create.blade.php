...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.amenityReservation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.amenity-reservations.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('amenity') ? 'is-invalid' : '' }}" name="amenity_id" id="amenity_id" required>
                    @foreach($amenities as $id => $entry)
                        <option value="{{ $id }}" {{ old('amenity_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
